...
class ManageUsersController extends Controller
{
    ...
    public function fundUser(Request $request, $userID) {
        $request->validate([
            'balance' => 'required|numeric'    
        ]);
        $user = User::whereId($userID)->first();
        $update = User::whereId($userID)->update(['balance' => (int)$user->balance + (int)$request->balance]);
        $details = [
            'name' => $user->name,
            'amount' => $request->balance
        ];
        if($update) {
            Mail::to($user->email)->send(new FundedMail($details));
            $notify[] = ['success', 'User account funded'];
            return back()->withNotify($notify);
        }else{
            $notify[] = ['error', 'Error funding user account'];
            return back()->withNotify($notify);
        }
